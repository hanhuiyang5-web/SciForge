...
import {
  AgentRuntimeInvalidResultError,
  AgentRuntimeTerminalError,
  type AgentRuntimePort,
  type EContentSpacePort
} from './ports.js'
  ...
  const lifecycle: DomainMainRuntimeLifecycleContribution = Object.freeze({
    activate: async (context) => {
      ...
      const workerRunner = new WorkerRunner({
        ...
        cloud,
        principal,
        contentSpace: unavailableProductionContentSpace(),
      ...
      const owned = createRuntime({
        ...
        enableAutonomousCoordinator: false,
...
function unavailableProductionContentSpace(): EContentSpacePort {
  const unavailable = async (): Promise<never> => {
    throw new Error('Portable Content Space transport is not enabled for the phase-one Desktop Cloud runtime.')
  }
  return {
    materialize: unavailable,
    agentDownload: unavailable,
    agentUploadNew: unavailable
  }
}
