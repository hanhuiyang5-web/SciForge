...
test('owner-direct phase one ignores Project planning triggers by default', async () => {
  let handler: Parameters<CollaborationBCNodePort['register']>[0] | undefined
  let coordinatorRecoveries = 0
  let coordinatorPlans = 0
  ...
  const runtime = new BCRuntime({
    ...
    coordinatorPlans: {
      list: async () => [],
      get: async () => { throw new Error('owner-direct mode must not read Coordinator plans') }
    } as unknown as FileCoordinatorPlanStore,
    coordinator: {
      recoverPendingWrites: async () => { coordinatorRecoveries += 1; return 0 },
      plan: async () => { coordinatorPlans += 1; throw new Error('must not plan') }
    } as unknown as Coordinator,
    workerRunner: {
      recoverPendingWrites: async () => 0,
    ...
    plannerFor: () => {
      plannerCreations += 1
      throw new Error('must not create a planner')
    }
  })
  ...
  assert.ok(handler)
  assert.deepEqual(
    await handler(projectStarted(), new AbortController().signal),
    { status: 'completed' }
  )
  assert.equal(coordinatorRecoveries, 0)
  assert.equal(coordinatorPlans, 0)
  assert.equal(plannerCreations, 0)
  await runtime.dispose()
})
...
test('B durably drives Project Plan through A confirmation to one idempotent Task creation', async () => {
  ...
  const runtime = new BCRuntime({
    ...
    enableAutonomousCoordinator: true,
