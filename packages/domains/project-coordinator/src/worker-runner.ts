...
import { AgentRuntimeInvalidResultError, AgentRuntimeTerminalError } from './ports.js'
import type {
  ACloudPort,
  AgentRuntimePort,
  CPrincipalPort,
...
export class WorkerRunner {
  ...
  private async runLocked(taskId: string, executionId: string, signal?: AbortSignal): Promise<void> {
    ...
    if (entry?.phase === 'succeeded' || entry?.phase === 'failed' || entry?.phase === 'rejected') return
    ...
    if (entry?.phase === 'terminal_queued') {
      ...
      await this.options.journal.save({
        ...entry,
        phase: terminalPhase(entry.terminalStatus),
        updatedAt: this.timestamp()
      })
      return
    }
    ...
    if (entry.phase === 'inputs_ready') {
      ...
      try {
        ...
        throwIfAborted(signal)
        if (
          error instanceof AgentRuntimeTerminalError ||
          error instanceof AgentRuntimeInvalidResultError
        ) {
          await this.reportAgentFailure(
            entry,
            principal.agentId,
            error instanceof AgentRuntimeTerminalError ? error.state : 'failed'
          )
          return
        }
        throw error
  ...
  private async rejectUnsupportedTask(
    entry: WorkerJournalEntry,
    task: Task,
    agentId: string,
    reason: string
  ): Promise<void> {
    const cloudKey = operationKey(`a-rejected:${task.taskId}:${task.executionId}:phase-one`)
    const command = {
      protocolVersion: '1.0' as const,
      requestId: operationRequestId(cloudKey),
      idempotencyKey: cloudKey,
...
function terminalPhase(status: WorkerJournalEntry['terminalStatus']): 'succeeded' | 'failed' | 'rejected' {
  if (status === 'succeeded' || status === 'failed' || status === 'rejected') return status
  throw new Error('Terminal journal entry has no terminal status.')
}
