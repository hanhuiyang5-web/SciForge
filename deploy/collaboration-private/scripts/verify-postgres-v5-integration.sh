...
[[ "$expected_schema_version" == 10 ]] \
  || die "The current PostgreSQL integration gate requires release schema v10 exactly."
...
list_integration_databases() {
  "${COMPOSE[@]}" exec -T postgres \
    ...
    --command="SELECT datname FROM pg_database WHERE datname ~ '^sciforge_identity_v10_it_[0-9]+_[0-9a-f]{12}$' ORDER BY datname;"
}

runner_script="$(canonical_regular_file "$SCRIPT_DIR/postgres-v5-integration.mjs")"
runner_container_id=""
runner_name="sciforge-postgres-v10-integration-${expected_commit:0:12}-$$"
password_file=""
...
cleanup() {
  ...
  if [[ "$cleanup_integration_databases" == true ]]; then
    if database_output="$(list_integration_databases)"; then
      ...
      for database in "${databases[@]}"; do
        [[ "$database" =~ ^sciforge_identity_v10_it_[0-9]+_[0-9a-f]{12}$ ]] \
  ...
  if [[ "$cleanup_failed" == true ]]; then
    echo "ERROR: PostgreSQL schema v10 integration cleanup was incomplete; operator inspection is required." >&2
    exit_status=1
  fi
  exit "$exit_status"
}
...
(( ${#preexisting_databases[@]} == 0 )) \
  || die "A stale PostgreSQL schema v10 integration database already exists; inspect it before a new run."
...
[[ "$password_file" =~ ^/run/sciforge-postgres-v5-admin\.[A-Za-z0-9]+$ \
  ...
  || die "Could not create the constrained PostgreSQL schema v10 password file."
dotenv_value "$ENV_FILE" SCIFORGE_COLLAB_DB_ADMIN_PASSWORD > "$password_file"
[[ "$(stat -c '%s' "$password_file")" == 64 ]] \
  || die "The PostgreSQL schema v10 password file has an invalid size."
grep -Eq '^[0-9A-Fa-f]{64}$' "$password_file" \
  || die "The PostgreSQL schema v10 password file has an invalid value."
chown root:10001 "$password_file"
chmod 0440 "$password_file"
[[ "$(stat -c '%u:%g:%a' "$password_file")" == 0:10001:440 ]] \
  || die "The PostgreSQL schema v10 password file must be root:10001 mode 0440."
...
[[ "$log_file" =~ ^/run/sciforge-postgres-v5-log\.[A-Za-z0-9]+$ \
  ...
  || die "Could not create the constrained PostgreSQL schema v10 log file."
chmod 0600 "$log_file"

runner_container_id="$(docker create \
  --name "$runner_name" \
  --label "org.opencontainers.image.revision=$expected_commit" \
  --label 'cn.sciforge.test.purpose=postgres-v10-integration' \
  --network "$database_network" \
  --user 10001:10001 \
  --read-only \
...
[[ -n "$runner_container_id" ]] || die "Could not create the PostgreSQL schema v10 integration runner."
...
[[ "$runner_image_id" == "$approved_image_id" \
  ...
  || die "The PostgreSQL schema v10 runner does not satisfy its fixed image, identity, network, mount, or secret boundary."
...
if (( wait_status != 0 )); then
  ...
  die "The PostgreSQL schema v10 integration runner timed out or could not be observed."
fi
[[ "$wait_output" =~ ^[0-9]+$ ]] || die "The PostgreSQL schema v10 runner returned an invalid exit status."
...
if grep -Fq -f "$password_file" "$log_file" \
  ...
  die "The PostgreSQL schema v10 runner log violated the secret redaction policy; its contents were suppressed."
fi
cat "$log_file"
[[ "$runner_exit_code" == 0 ]] || die "The PostgreSQL schema v10 integration runner failed."
grep -Fq '"event":"postgres.v10.integration","status":"passed"' "$log_file" \
  || die "The PostgreSQL schema v10 integration runner did not emit its pass receipt."
for required_check in v1_to_v5_to_v10_readiness portal_bounded_read_indexes portal_hard_caps \
  provider_identity_inbox_constraint \
  portable_resource_reference_constraint managed_provider_container_schema \
  project_content_space_task_io_schema \
  legacy_agent_revocation legacy_project_record_author_backfill \
  legacy_project_record_author_transfer_ambiguity concurrent_oidc_jit \
  device_agent_lifecycle zulip_binding_uniqueness; do
  grep -Fq "\"$required_check\"" "$log_file" \
    || die "The PostgreSQL schema v10 pass receipt is missing a required check."
done
for required_index in agent_nodes_active_owner_agent_idx \
  human_answers_project_created_answer_idx \
  tasks_project_task_id_idx project_records_project_record_id_idx \
  human_requests_project_target_request_id_idx tasks_active_assignee_idx \
  oidc_identities_active_user_issuer_idx project_members_active_user_project_idx \
  project_members_active_project_user_idx project_records_candidate_task_result_project_idx; do
  grep -Fq "\"$required_index\"" "$log_file" \
    || die "The PostgreSQL schema v10 pass receipt is missing a required Portal bounded-read index."
done
...
[[ "$production_snapshot_after" == "$production_snapshot_before" ]] \
  || die "Production schema versions, actual tables, row counts, or content digests changed during the isolated v10 integration run."
[[ "$app_state_after" == "$app_state_before" ]] \
  || die "The live app container ID, host PID, RestartCount, image, or revision changed during the isolated v10 integration run."
curl --fail --silent --show-error --max-time 5 "$base_url/healthz" > /dev/null
curl --fail --silent --show-error --max-time 5 "$base_url/readyz" > /dev/null
...
for digest in "$release_manifest_digest" "$bundle_sums_digest" "$bundle_commit_digest" \
  ...
  [[ "$digest" =~ ^[0-9a-f]{64}$ ]] || die "Could not derive a schema v10 attestation input digest."
done
...
[[ "$verified_epoch" =~ ^[0-9]{10,}$ \
  ...
  || die "Could not derive the schema v10 attestation time."
...
[[ "$attestation_temporary" =~ ^/run/sciforge-postgres-v5-attestation\.[A-Za-z0-9]+$ \
  ...
  || die "Could not create the PostgreSQL schema v10 attestation."
...
[[ "$(stat -c '%u:%g:%a' "$attestation_temporary")" == 0:0:600 ]] \
  || die "The PostgreSQL schema v10 attestation must be root:root mode 0600."
mv -f -- "$attestation_temporary" "$attestation_path"
attestation_temporary=""

echo "PostgreSQL schema v10 integration passed through the v5 baseline in a random isolated database, including the Project Content Space Task I/O schema, fixed Portal hard caps, and all ten Portal/coordination bounded-read indexes; the production consistency snapshot and live app state were unchanged, and the compatibility-named one-time deployment attestation was written."
